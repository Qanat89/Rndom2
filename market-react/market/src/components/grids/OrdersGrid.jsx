...
function OrdersGrid({date, time, status, price, products}) {
  ...
  return (
    ...
    { allList && products.slice(0, 4).map((x, key)=>(<ProductCard count={x.count} key={key} id={x.id} price={x.price} priceOriginal={x.priceOriginal} discount={x.discount} title={x.title} img={x.img}/>))
    }
    </div>
